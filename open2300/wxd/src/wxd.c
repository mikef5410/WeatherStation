...
int get_curwx(WEATHERSTATION ws)
{
	...
	if ( lastrain == 0 ) lastrain=curtime;
	  ...
	  //Do we have rain? Or did 60min expire so we can average in zero rain rate?
	  if  ((dt>0 && dr>1.0E-5)||(dt>=3600)) {
	    ...
	    rravg = (rravg < 1.0E-5) ? 0.0 : rravg;
	    update(current_obs.rain_rate, (rravg<1.0E-2 ? 0.0 : rravg) , RAINRATE);
	    lastrain=curtime;
	  }
        }
