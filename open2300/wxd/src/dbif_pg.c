...
db_writer(void)
{
    ...
    if (ch_flag != 0) { //update table current_obs
        sprintf(sql_str, "BEGIN; UPDATE current_obs SET ");
        ix = strlen(sql_str);

        if (test_bit(ch_flag, RAINTOT)) {
	    sprintf(&sql_str[ix], "rain_tot = %.3f, rtot_offset = %.3f ", current_obs.rain_tot, current_obs.rtot_offset);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c rain_rate = %.3f ", comma, current_obs.rain_rate);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c temp_in = %.3f ", comma, current_obs.temp_in);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c temp_out = %.3f ", comma, current_obs.temp_out);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c chill = %.3f ", comma, current_obs.chill);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c dp_in = %.3f ", comma, current_obs.dp_in);
	    ix = strlen(sql_str);
	    didone++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c dp_out = %.3f ", comma, current_obs.dp_out);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c rh_in = %.3f ", comma, current_obs.rh_in);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c rh_out = %.3f ", comma, current_obs.rh_out);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c barometer = %.3f ", comma, current_obs.barometer);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c gust_spd = %.3f ", comma, current_obs.gust);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
	    ...
	    comma = didone ? ',' : ' ';
	    sprintf(&sql_str[ix], "%c wavg_spd = %.3f ", comma, current_obs.wavg);
	    ix = strlen(sql_str);
	    didone++;
	    need_write++;
    ...
    curtime = time(NULL);
    if (((curtime - last_write_time) > WRITE_INTERVAL) || need_write) { //insert into table wxobs
        write_obs();
...
init_db(void)
{
   ...
   dbres = PQexec(dbconn, "select * from current_obs limit 1;");
   ...
   current_obs.rh_in = atof(db_getval(tup, "rh_in"));
   current_obs.rh_out = atof(db_getval(tup, "rh_out"));
   ...
   current_obs.rtot_offset = atof(db_getval(tup, "rtot_offset"));
   current_obs.rain_tot = atof(db_getval(tup, "rain_tot"));
   current_obs.rain_rate = atof(db_getval(tup, "rain_rate"));
