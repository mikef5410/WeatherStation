...
sub historical {
  ...
  printf("<b>Rain $period:</b> %4.4g in<br>\n",mmtoin($today_rain));
  if ($toprate) {
    printf("<b>Peak rain rate $period:</b> %4.2g in/hr at %s<br>\n",mmtoin($toprate),$toptime);
  }
}
...
sub temp_extreme {
  ...
  printf("<b>High temp $period:</b> %4g at %s<br>\n",CtoF($toptemp),$toptime);
  printf("<b>Low  temp $period:</b> %4g at %s<br>\n",CtoF($mintemp),$mintime);
}
